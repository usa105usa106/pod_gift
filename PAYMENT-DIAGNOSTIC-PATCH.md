# Payment diagnostics and form lifecycle — v0033

v0033 hardens FAST paid-upgrade volleys and keeps payment evidence for later debugging.
...
- Refresh is staggered in small batches while LIVE is active, so a scanner that runs for weeks or months does not keep the form created at startup.
...
- Dedicated structured audit: `gift-hunter-v0033-payment-audit.jsonl`; `/log_full` exports only its last 24 hours, can split oversized ZIPs, and clears local history only after every part is sent successfully.
